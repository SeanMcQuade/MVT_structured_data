...
Analyses that used to be run by hand are now pipeline stages, with the same
staleness checking, sharding and `make` targets as the rest of the pipeline.
Each pairs a data stage that does the expensive work with a plotting stage that
reads its output, so the figures can be rebuilt without the trajectories.
...
| `relspeed` | `relative_speed_histogram.m` | slim JSON | `relspeed_data_DD.mat` |
| `relspeedplot` | `plot_relative_speed.m`, `binned_relative_speed.m` | `relspeed_data_DD.mat` | 2 × `fig_relspeed_*.pdf` per day |

The `.mat` products land in `results/analysis/2022-11-DD/` and the figures in
`results/figures/2022-11-DD/`, so the inputs to a figure can be distributed
without the figure.
...
make relspeed-17               # pool the day's relative-speed samples
make relspeedplot-17           # the two relative-speed figures
make lanes lc lcplot relspeed relspeedplot   # all of them, all days in DAYS
make status                    # what is stale, and why
```

`lanes`, `lc` and `relspeed` are part of `make data`; `lcplot` and
`relspeedplot` are part of `make figures`, and therefore of `make all`. From inside MATLAB, use
`mvt.build('lcplot', 17)` or let `run_all_scripts` walk the whole graph.

Only `lanes` shards, because it is the only one that works segment by segment. `lc` deliberately processes a whole day in one process: it decodes
...
make RESULTS=/path/to/results_test lc-17 lcplot-17 relspeed-17 relspeedplot-17
```
...
```matlab
mvt.matHash('/path/to/results_test/analysis/2022-11-17/LC_data_17.mat')
mvt.matHash('/path/to/results/analysis/2022-11-17/LC_data_17.mat')
```
...
**Figure canvases are pinned.** `plot_relative_speed` and
