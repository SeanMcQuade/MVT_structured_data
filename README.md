...
# Use of generative AI for code

The original version of the data were produced through hand-written code only. To improve computation time, the use of generative AI tools were applied to enable parallel computation through makefiles, and to create improved HOWTO documentation. All files generated by the generative AI software versions match perfectly through comparisons with the md5sum to the original analysis files. 

The python libraries to visualize and produce similar plots were also required to match bitwise through md5sum comparisons when they generated their intermediate data, to confirm that the libraries were exactly producing the same results.

Thus the generative AI tools improved the reproducibility and speed of computation, while ensuring that the results were identical to those performed from expert-written code.
