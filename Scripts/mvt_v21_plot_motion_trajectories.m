...
zoom_t_rel = [.44 .49]; zoom_x_rel = [.13 .23]; % zoom window coords
flag_save_lowres = 0; % true if low resolution figures should be save
flag_save_highres = 1; % true if high resolution figures should be save
...
for file_j = 1:length(data_files) % loop over relevant files
    ...
    ind = find(([data.ending_x]-[data.starting_x])*direction>0&([data.lane_number]==lane|lane==0));
    ...
    for j = ind % loop over used trajectories
        traj_len = data(j).length*0.3048; % length of vehicle [m]
        ...
        patch([traj_t;traj_t(end:-1:1)],...
            [traj_x;traj_x(end:-1:1)+traj_len*direction]/1000,...
