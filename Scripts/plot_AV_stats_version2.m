% script to plot multiple statistics for AV fuel consumption for experiments run on the following dates:
...
%% Create triplets of mean, median, effective Fuel Consumption, then partition them according to distance from AV ahead, Masked ahead, masked behind, behind
...
load ../Data/Data_for_Figures/data_av_activation.mat % Load Variables

t_cell_array = {t_16 t_17 t_18};
x_cell_array = {x_16 x_17 x_18};
data_cell_array = {data_16 data_17 data_18};

fields_filenames = ["fields_motion_2022-11-16" "fields_motion_2022-11-17" "fields_motion_2022-11-18"];%bulk traffic speeds
Fig_string = ["bulkSpeedAVs1" "bulkSpeedAVs2" "bulkSpeedAVs3"];
for i=1:3 %wed. thurs. fri.
    data_path = ['../Data/Data_for_Figures/', fields_filenames{i}];
    load(data_path)
    ...
    for ii = 1:length(t)
        ...
        u_temp_mean(i,ii) = sum(u_temp_v .* rho_temp_v)/total_rho_temp;
        ...
        nAVs_active(i,ii) = nAVs_active_temp;
        nAVs(i,ii) = nAVs_temp;
    end

    %% FIGURE 1 Plotting bulk speed
    % colors
    ...
    TT{i} = datetime(t, 'convertfrom', 'posixtime', 'Format', 'HH:mm:ss.SSS','TimeZone' ,'America/Chicago') ; %[7:00:00,9:00:00;7:20:00,8:50:00;7:00:00,8:30:00]
    plot_start_t(i) = datetime(2022,11,test_days(i), str2num(Firsthour), str2num(Firstminutes),0, 'TimeZone' ,'America/Chicago');
    plot_end_t(i) = datetime(2022,11,test_days(i), str2num(Secondhour), str2num(Secondminutes),0, 'TimeZone' ,'America/Chicago');
    twindow_nAVs_active(i,:) = nAVs_active(i,TT{i}>plot_start_t(i));
...
for i = 1:3
    ...
    plot(TT{i}(TT{i} <= plot_start_t(i)), nAVs_active(i,TT{i} <= plot_start_t(i)),'Color',lightred,'LineStyle','-','LineWidth',1.5)
    plot(TT{i}(TT{i} >= plot_end_t(i)), nAVs_active(i,TT{i} >= plot_end_t(i)),'Color',lightred,'LineStyle','-','LineWidth',1.5)
    plot(TT{i}(TT{i} >= plot_start_t(i) & TT{i} <= plot_end_t(i)), nAVs_active(i,TT{i} >= plot_start_t(i) & TT{i} <= plot_end_t(i)),'r-','LineWidth',1.5)
    plot([plot_start_t(i) plot_start_t(i)],[0,55],'k:','LineWidth',1.5) %vertical lines
    plot([plot_end_t(i) plot_end_t(i)],[0,55],'k:','LineWidth',1.5)
    ylabel('Number of active AVs')
    set(gca,'YColor','r');
    grid on
    x_range=[plot_start_t(i)-minutes(30),plot_end_t(i)+minutes(30)];
    y_range = [0,55];
    title('Traffic state and AV deployment vs. time')
    text(plot_start_t(i)+minutes(2),(y_range(2) - y_range(1))*0.9,'start time','FontSize',fontsize)
    text(plot_end_t(i)-minutes(47),(y_range(2) - y_range(1))*0.9,'end time','FontSize',fontsize)
    ...
    yyaxis left
    plot(TT{i}(TT{i} <= plot_start_t(i)), u_temp_mean(i,TT{i} <= plot_start_t(i)),'Color',lightblue,'LineStyle','-','LineWidth',2)
    plot(TT{i}(TT{i} >= plot_end_t(i)), u_temp_mean(i,TT{i} >= plot_end_t(i)),'Color',lightblue,'LineStyle','-','LineWidth',2)
    plot(TT{i}(TT{i} >= plot_start_t(i) & TT{i} <= plot_end_t(i)), u_temp_mean(i,TT{i} >= plot_start_t(i) & TT{i} <= plot_end_t(i)),'Color','b','LineStyle','-','LineWidth',2)
