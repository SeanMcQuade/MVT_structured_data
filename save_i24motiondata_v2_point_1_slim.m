% save_i24motiondata_v2_point_1_slim - (C) CIRCLES Energy team, 2/25/2025
% 
...
%
% Make sure just one day of json data files are in the current MATLAB path.
% Generated json files will be saved in \Results folder.

clear
mkdir('Results');
% Set processing parameters
processingOpts.laneIdentificationOpts.LANEWIDTH = 12;       % Lane width in feet
processingOpts.laneIdentificationOpts.Nr_XCELLS = 200;      % Number of cells in the x direction used to identify driving line
processingOpts.laneIdentificationOpts.MINCELLSAMPLES = 20;  % Minimum number of samples per cell used to identify driving line
processingOpts.laneIdentificationOpts.Y_UP_LIM = 5;        % samples upper bound as a multiplier of lane width to remove outliers
processingOpts.laneIdentificationOpts.Y_LOW_LIM = 0.5;        % samples lower bound as a multiplier of lane width to remove outliers
processingOpts.laneIdentificationOpts.Se = 0.97;            % Shift parameters for y position correction (westbound) s.t. y_corrected =  Sw * (y  - drivingLineShift_w) + Cw;
processingOpts.laneIdentificationOpts.Ce = 1;
processingOpts.laneIdentificationOpts.Sw = 0.98;            % Shift parameters for y position correction (eastbound) s.t. y_corrected =  Se * (y  - drivingLineShift_e) + Ce;
processingOpts.laneIdentificationOpts.Cw = 1;

processingOpts.laneChangeClippingOpts.LANECHANGETHRSH = 0.5;        % Threshold to identify lane change as a multiplier of lane width 
processingOpts.laneChangeClippingOpts.MAXLANECHANGERATE = 0.1;      % Threshold for maximum rate of change in the identified lane for which trajectory is assumed to not change lane as a multiplier of lane width/s
processingOpts.laneChangeClippingOpts.MINCLIPTIME = 0.5;            % Minimum duration for a clipped trajectory in seconds
processingOpts.laneChangeClippingOpts.CHANGEBUFFERTHRESH = 0.2;     % Buffer threshold to ensure full lane change as a multiplier of lane width 

% Load road grade map
gradeData = readmatrix('Eastbound_grade_fit.csv');
gradeDataStart = gradeData(:,2); %0.225miles is the distance between mill creek origin (MM58.675) and MM58.9 the estimated origin of the road grade map
...
% Identify MVT day to be processed 
[dayToProcess, baseFileName] = findmvtdatafiles();


%%% GPS Data is assumed to be processed. Follow process_GPS_data_v2_point_1.m to produce processed GPS files      
dataGPS = jsondecode(fileread(['CIRCLES_GPS_10Hz_2022-11-' num2str(dayToProcess) '__UPDATE.json'])); 
    
for fileNr = 1:24
    
    if fileNr <11
        filenameLoad = [baseFileName,'0',char(num2str(fileNr-1)) ,'.json'];
    else
        filenameLoad = [baseFileName,char(num2str(fileNr-1)) ,'.json'];
    end

    tic
    fprintf(sprintf('\n Loading original data file, %d/24 \n', fileNr))    
    dataTemp = jsondecode(fileread(filenameLoad));
    dataTemp = dataTemp([dataTemp.direction]<0); % remove eastbound trajectories 
    dataTemp = rmfield(dataTemp,{'flags','compute_node_id','fragment_ids','merged_ids',...
        'configuration_id','fine_vehicle_class','x_score','y_score','road_segment_ids'});% delete unnecessary fields, note:'camera_snap_shots' not available for all test days 
    
    % Identify and assign lanes 
    
    dataLanes = assignlanes(dataTemp,processingOpts.laneIdentificationOpts);
    
    % Clip data to remove lane switches 
    
    dataTemp = cliplanechanges(dataTemp,dataLanes,processingOpts.laneChangeClippingOpts);
    
    fprintf('\n Calculating Distance to AVs\n')
    distToAvsData = calculatedistancetoavs(dataTemp,dataGPS);
    
    
    
    fprintf('\n Generating v2.1 version of data\n')
    fprintf('_______20%%_______40%%_______60%%_______80%%______100%%\n')
    n = length(dataTemp);
    data = initDataStruct(n);
    for i=1:floor(n) %loop over all data(vehicle) segments
    
        if floor(i/n*50)>floor((i-1)/n*50) % progress marker
           fprintf('^')
        end
        ...
        data(i).coarse_vehicle_class = veh.coarse_vehicle_class; % numbers corresponding to vehicle types 0:sedan, 1:midSUV, 2:van, 3:pickup, 4:semi, 5:truck
        ...
        data(i).lane_number = veh.lane; %lane number of the vehicle: 1 leftmost lane(HOV) , 4 right most lane. 0 off the highway, 5 on/off ramp.
        
        data(i).x_position_meters = 0.3048*(veh.x_position-309804.125); % shift x position to v2 local coordinate system (origin: Mill Creek Bridge) and convert to meters.
        ...
        v = (x([2:end,end])-x([1,1:end-1]))./(t([2:end,end])-t([1,1:end-1])); % Calculate velocity (m/s)
        a = (x(1:end-2)-2*x(2:end-1)+x(3:end))./((t(3:end)-t(1:end-2))/2).^2; % Calculate acceleration central (m/s/s)
        a = a([1,1:end,end]); 
        ...
        % shift = 6.01*1609.344 + 309804.5*0.3048; % approx distance between symernia and origin in nashville city (6.01mil is distance between Sym and mm60 and 309804.5 is distance from mm to origin of I24motion system near Nashville city)
        % xNew = shift-0.3048*veh.x_position;     % position of vehicles wrt to symernia origin (end of Sam Ridely Pkwy on ramp)
        ...
        data(i).total_fuel_economy_mpg = ((data(i).total_distance_traversed_meters)/1609.34)./(data(i).total_fuel_consumed_gallons);
    ...
    filenameSave = ['Results\I-24MOTION_slim_',fileStartT];
...
function [dayToProcess, baseFileName] = findmvtdatafiles()

    files_in_dir = dir( '648*.json');
    if length(files_in_dir) > 24
        error('Error identifying I24 json files. Make sure only a full day of v2 data files are in the workspace directory.');
    end
    file_names = vertcat(files_in_dir.name);
    ind_d = strfind(file_names(1,:),'__')+2;
    ind_d = [ind_d:ind_d+2];
    day_s = string(file_names(1,ind_d));
    if sum(strcmp(string(file_names(:,ind_d)) ,day_s )) ~= size(file_names,1)
        error('Data files in the folder are from different days.  Make sure only a full day of v2 data files are in the workspace directory.')
    end
    ...
    baseFileName = file_names(1,1:end-7);
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function data = initDataStruct(n)
data(1,n) = struct('trajectory_id', [],'timestamp',[],'x_position_meters',[],'y_position_corrected_meters',[],...
   ...
   'reference_fuel_rate_flat_road_grams_per_second',[],'percent_reference_infeasibility_flat_road',[],'total_reference_fuel_consumed_flat_road_grams',[],...
...
function [dataLanes] = assignlanes(baseFileName,laneIdentificationOpts)
...
%
    ...
    %%% define Driving Line
    fprintf('Sampling Data for Lane Identification \n')
    % sample data
    N = length(data);
    xWest = []; yWest = [];
    fprintf('_______20%%_______40%%_______60%%_______80%%______100%%\n')
    ...
    for i = 1:Nr_XCELLS
        if floor(i/Nr_XCELLS*50)>floor((i-1)/Nr_XCELLS*50) % progress marker
           fprintf('^')
        end
...
function newData = cliplanechanges(data,dataLanes,opts)
...
%   
    ...
    fprintf('Clipping lane changes in the data \n')
    fprintf('_______20%%_______40%%_______60%%_______80%%______100%%\n')
    
    trajCounter =1;
    for i = 1:nrTrajs
        
        if floor(i/nrTrajs*50)>floor((i-1)/nrTrajs*50) % progress marker
           fprintf('^')
        end
...
function DistToAvs = calculatedistancetoavs(dataTemp,dataGPS)
...
%             distanceToAvDSEng: distance to nearest engaged AV downstream at each timestamp
    ...
    fprintf('_______20%%_______40%%_______60%%_______80%%______100%%\n')
    for i = 1:num_trajs
        if floor(i/num_trajs*50)>floor((i-1)/num_trajs*50) % progress marker
           fprintf('^')
        end
