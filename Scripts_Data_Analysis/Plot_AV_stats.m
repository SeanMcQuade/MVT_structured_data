...
TWINDOWS = [0645, 0915]; % time windows of intrest in military time
flagSave = 1; % save figures
...
for iTWin = 1:size(TWINDOWS,1)
    ...
    for i=1:length(testDays) % three days, Wed thrus fri
        ...
        ind = loadedData.samples_speed >= minSpdRaw; %set to '0' to keep all speed samples
    ...
    % Create triplets of mean median eff
    ...
    if flagSave
        fname = strjoin([parentDirectory '\Figures\fig_3_fuel_results_' strjoin(statsToPlotChoice(:),'_') ...
            '_',TWINDOWSTR(1),TWINDOWSTR(2),TWINDOWSTR(3),'_',TWINDOWSTR(6),...
            TWINDOWSTR(7),TWINDOWSTR(8)],'');
